...
- Single-writer fence: `enable_writes` false returns `WRITE_BLOCKED` (`requested_operation`, `source`, `controller_state`, `enable_writes`, `reason`, `network_write_sent=false`) before a Braiins request is built, including an unbound client. Inventory: `docs/phase1-writer-inventory.md`. Home Assistant automations are listed only as a live-inventory template.
